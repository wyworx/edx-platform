...
def student_dashboard(request):
    ...
    if not user.is_active:
        activate_account_message = Text(_(
            ...
            "If you need help, contact admin@5thwork.com."
        )).format(
            platform_name=platform_name,
    ...
    context = {
        ...
        'display_download_centbee_mobile_app_message': not UserProfile.objects.filter(user=user).first().public_address,
        'display_add_public_address_message': not UserProfile.objects.filter(user=user).first().public_address,
